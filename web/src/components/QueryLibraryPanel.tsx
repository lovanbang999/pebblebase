...
import type { SavedQuery, SavedQueryUpdateInput } from "../lib/types";
import {
  fetchSavedQueries,
  updateSavedQuery,
  deleteSavedQuery,
...
export default function QueryLibraryPanel({
  ...
  const load = useCallback(async () => {
    ...
    try {
      const data = await fetchSavedQueries(connectionId, {
        ...
        tag: activeTag !== "__all__" ? activeTag : undefined,
      });
      setQueries(data);
    } catch {
      setQueries([]);
  ...
  }, [connectionId, searchTerm, activeTag]);
  ...
  const renderQueryItem = (q: SavedQuery) => (
    ...
    >
      ...
      <div className="flex items-start gap-1.5 min-w-0">
        {/* Favorite star — always visible if starred */}
        {q.is_favorite && (
      ...
      {allTags.length > 0 && (
        <div className="flex flex-wrap gap-1 px-2.5 py-1.5 border-b border-white/5">
          ...
          {allTags.map((tag) => (
            <button
              key={tag}
              onClick={() => setActiveTag(activeTag === tag ? "__all__" : tag)}
              className={cn(
                "px-2 py-0.5 rounded-full text-[9px] font-bold border transition-all",
                activeTag === tag
      ...
      <div className="flex-1 overflow-y-auto">
        ...
        {queries.length === 0 && !loading && (
          <div className="flex flex-col items-center justify-center gap-3 h-full px-4 py-10 text-center">
            {/* Decorative illustration-style icon */}
            <div className="relative">
