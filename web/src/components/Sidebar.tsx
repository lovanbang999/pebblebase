...
const ENGINE_CONFIG: { type: DatabaseType; label: string; badge: string }[] = [
  { type: 'postgres', label: 'PostgreSQL', badge: 'bg-indigo-500/10 text-indigo-700 dark:text-indigo-400 border-indigo-500/20' },
  { type: 'mysql', label: 'MySQL', badge: 'bg-amber-500/10 text-amber-700 dark:text-amber-400 border-amber-500/20' },
  { type: 'mongodb', label: 'MongoDB', badge: 'bg-emerald-500/10 text-emerald-700 dark:text-emerald-400 border-emerald-500/20' },
];
...
export const Sidebar: FC<SidebarProps> = ({
  ...
  const isDeleteConfirmed = Boolean(
    deletingConnection && deleteConfirmationInput.trim() === deletingConnection.name.trim()
  );
              ...
              >
                ...
                <div className="max-h-60 overflow-y-auto px-1.5 py-1.5 custom-scrollbar">
                  {ENGINE_CONFIG.map(({ type, label, badge }) => {
                    const engineConns = connections.filter((c) => c.type === type);
                    ...
                    return (
                      <div key={type} className="mb-2.5 last:mb-0.5">
                        <div className="px-2 py-1 text-[10px] font-mono uppercase tracking-wider text-muted-foreground font-semibold flex items-center justify-between bg-zinc-100/50 dark:bg-zinc-900/40 rounded mb-1">
                          <Badge variant="outline" className={cn("text-[9px] px-1.5 py-0 h-4 font-mono", badge)}>
                            {label}
                          </Badge>
                            ...
                            >
                              <div className="flex items-center gap-2.5 truncate flex-1 min-w-0">
                                {isCurrent ? (
                                  ...
                                  <span
                                    className={cn("size-2 rounded-full shrink-0 ring-1", dotColor)}
                    ...
                    <SidebarMenuItem key={tbl.name}>
                      ...
                      >
                        ...
                        <div className="ml-auto flex items-center gap-1 shrink-0 group-data-[collapsible=icon]:hidden">
                          ...
                          >
                            <span className="inline-block translate-y-px leading-none">{colCount}</span>
                          </Badge>
                        </div>
                      </SidebarMenuButton>
                    </SidebarMenuItem>
        ...
        <AlertDialogContent className="bg-white dark:bg-zinc-950 border border-zinc-200 dark:border-zinc-800 rounded-xl shadow-2xl">
          <AlertDialogHeader>
            ...
            <AlertDialogDescription className="text-xs text-zinc-500 dark:text-zinc-400 leading-relaxed">
              This action cannot be undone. This will permanently remove the connection profile <span className="font-semibold text-zinc-900 dark:text-zinc-100 font-mono">"{deletingConnection?.name}"</span> from Pebblebase. Your actual database and data will not be affected.
            </AlertDialogDescription>
          </AlertDialogHeader>
