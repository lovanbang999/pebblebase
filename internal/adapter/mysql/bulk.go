...
func streamTableRows(
	...
	if err != nil {
		return fmt.Errorf("mysql: stream query %q: %w", table, err)
	}
	defer rows.Close()
	...
	for rows.Next() {
		...
		if err := rows.Scan(valuePtrs...); err != nil {
			return fmt.Errorf("mysql: stream scan: %w", err)
		}
	...
	if err := rows.Err(); err != nil {
		return fmt.Errorf("mysql: stream iterate: %w", err)
	}
...
// bulkInsert inserts rows within an atomic MySQL database transaction.
func bulkInsert(
	ctx context.Context,
	db *sql.DB,
	table string,
	...
	if err != nil {
		return 0, fmt.Errorf("mysql: begin bulk tx: %w", err)
	}
	defer tx.Rollback()
	...
	for i := 0; i < len(records); i += maxBatch {
		...
		if err != nil {
			return 0, fmt.Errorf("mysql: bulk insert batch rows [%d-%d]: %w", i+1, end, err)
		}
	...
	if err := tx.Commit(); err != nil {
		return 0, fmt.Errorf("mysql: commit bulk tx: %w", err)
	}
