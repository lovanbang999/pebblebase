...
func executeRaw(ctx context.Context, db *sql.DB, query string) (adapter.RawQueryResult, error) {
	...
	if isQueryLike {
		...
		if err == nil {
			...
			if err == nil && len(colNames) > 0 {
				...
				for rows.Next() {
					...
					if err := rows.Scan(valuePtrs...); err != nil {
						return adapter.RawQueryResult{}, fmt.Errorf("sqlite: scan row: %w", err)
					}
				...
				if err := rows.Err(); err != nil {
					return adapter.RawQueryResult{}, fmt.Errorf("sqlite: iterate rows: %w", err)
				}
				if result == nil {
			...
			return adapter.RawQueryResult{}, fmt.Errorf("sqlite: query error: %w", err)
		}
	}

	res, err := db.ExecContext(ctx, cleanQuery)
	if err != nil {
		return adapter.RawQueryResult{}, fmt.Errorf("sqlite: exec error: %w", err)
	}
...
func isMutationSQL(upper string) bool {
	for _, kw := range []string{"INSERT", "UPDATE", "DELETE", "DROP", "ALTER", "CREATE", "TRUNCATE", "REPLACE", "RENAME", "ATTACH", "DETACH"} {
