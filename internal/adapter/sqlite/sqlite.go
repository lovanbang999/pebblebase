...
func (a *SQLiteAdapter) StreamRows(ctx context.Context, table string, opts adapter.QueryOptions, onChunk func(columns []string, rows []map[string]any) error) error {
	return streamTableRows(ctx, a.db, table, opts, onChunk)
}

// BulkInsert transactionally inserts multiple records in batches.
func (a *SQLiteAdapter) BulkInsert(ctx context.Context, table string, columns []string, records [][]any) (int64, error) {
